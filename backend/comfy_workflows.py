...
def extract_from_png(data: bytes | str | Path) -> dict:
    ...
    if not raw:
        ...
        if info.get("workflow"):
            raise ValueError(
                "This PNG only carries the workflow in UI format — the API format "
                "is required. Try another ComfyUI image or export the "
                "workflow manually (Save → API Format)."
            )
        raise ValueError(
            "The PNG carries no ComfyUI workflow ('prompt' metadata missing). "
            "Was this image really generated by ComfyUI?"
        )
...
def _ensure_api_format(obj: Any) -> dict:
    if not isinstance(obj, dict):
        raise ValueError("The workflow must be a JSON object.")
    # UI format has nodes + links arrays at the top level.
    if "nodes" in obj and "links" in obj:
        raise ValueError(
            "This is a UI-format workflow (with 'nodes' and 'links' fields), but "
            "the API format is required. Easiest fix: just drop any image "
            "generated by ComfyUI into the app — the workflow is embedded in "
            "its metadata. Or in ComfyUI: settings → Enable Dev Mode → "
            "Save (API Format)."
        )
    if not obj:
    ...
    if not isinstance(sample, dict) or "class_type" not in sample:
        raise ValueError("This does not look like the ComfyUI API format.")
    return obj
